...
class BasicCog(commands.Cog):
    ...
    async def status(self, ctx):
        ...
        await ctx.response.send_message(embed=EmbedManager(ctx).generate(
            embed_title=f"{config.bot.name}のステータス",
            embed_content=[{
                "title": "起動時間",
                "value": f"{uptime}"
            }],
    ...
    async def reload(self, ctx):
        ...
        await ctx.response.send_message(embed=EmbedManager(ctx).generate(
            mode="succeed",
            embed_title=i18n.t("message.system.reloadSuccess", locale="ja"),
        ).embed)
        # await ctx.response.send_message
    ...
    async def reload_error(self, ctx, error):
        if isinstance(error, commands.CheckFailure):
            await EmbedManager(ctx).generate(
                mode="succeed",
                embed_title="エラー",
                embed_description="このコマンドはBotの所有者のみが実行できます",
            ).send()
    ...
    async def set(self, ctx, language: Language):
        ...
        await ctx.response.send_message(embed=EmbedManager(ctx).generate(
            mode="succeed",
            embed_title=i18n.t("message.locale.ChangeSuccessTitle",
                               locale=language),
        ).embed)

    @commands.slash_command(
        name="random",
        description="1から100までのランダムな数値を生成します。オプションで最小/最大値を変更できます",
        guild_ids=config.options.slash_command_guild,
    )
    ...
    async def translate(self,
        ...
        """
        await ctx.response.send_message(translator(content, from_lang,
                                                   to_lang))
