...
from NextHime import config, db_manager, session
...
class BlogCog(commands.Cog):
    ...
    async def setup_category(self, ctx: ApplicationCommandInteraction):
        ...
        await ctx.response.send_message(embed=EmbedManager(ctx=ctx).generate(
            "成功", f"{ctx.channel.category.name}`をブログカテゴリとして登録しました",
            "success").embed)

    @setup.sub_command(name="replay", description="ブログに対する返信チャンネルを設定します")
    async def setup_replay_channel(self,
        ...
        guild = (session.query(GuildBlogs).filter(
            GuildBlogs.guild_id == ctx.guild.id).first())
        guild.replay_channel_id = ctx.channel.id
        session.commit()
        await ctx.response.send_message(embed=EmbedManager(ctx=ctx).generate(
            "成功", f"<#{channel_id}>を返信チャンネルとして登録しました", "success").embed)
    ...
    async def on_message(self, ctx: disnake.Message):
        if ctx.author.bot:
            return
        print(ctx.content)
        category = (session.query(BlogsCategory).filter(
            BlogsCategory.guild_id == ctx.guild.id).first())
        if category:
            replay_channel = (session.query(GuildBlogs).filter(
                and_(
                    GuildBlogs.guild_id == ctx.guild.id,
                    GuildBlogs.replay_channel_id == ctx.channel.id,
                )).first())
