...
class WarframeCog(commands.Cog):
    ...
    async def bot_warframe_loop(self):
        fissure_list = (WarframeAPI().get_fissures().save_fissure_json(
            tempfile.gettempdir() + "/fissure_list.json", check=True))
        ...
        for fissure_dict in fissure_list:
            ...
            if not get_old_fissures:
                await WarframeChannels(self.bot,
                                       fissure).send_message(send_channels)
                redis_conn.set(f"fissure_{fissure.id}",
                               json.dumps(fissure_dict))

    @commands.slash_command(name="warframe",
    ...
    async def setup(self, ctx):
        await db_manager.commit(
            Guilds(server_id=ctx.guild.id, region=ctx.guild.region[0]))
        await db_manager.commit(
            WarframeFissuresChannel(channel_id=ctx.channel.id,
                                    region=ctx.guild.region[0]))
        await ctx.response.send_message(
            embed=EmbedManager(ctx).generate("セットアップに成功", mode="success").embed
        )
