...
class AutoMigrate(object):
    def __init__(self):
        ...
        self.spinner = Halo(
            text="Loading Now",
            spinner={"interval": 300, "frames": ["-", "+", "o", "+", "-"]},
        )
        self.logger = EasyLogger(
            getLogger("auto_migrate"), logger_level="DEBUG"
        ).create()
    ...
    def process(self, commands: list = None):
        ...
        for log in logs:
            if (
                ...
                or re.search("INFO\s(.*)\sRunning\supgrade\s(.*)\s->\s(.*),\s(.*)", log)
    ...
    def generate(self):
        ...
        if generate_status == 0:
            ...
            self.spinner.fail(
                "データベースのバージョン管理システムとの整合性が損なわれています\n再度実行し問題が修正されない場合は作者へご連絡ください"
            )
            sys.exit(1)
