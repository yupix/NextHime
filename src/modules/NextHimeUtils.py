...
class NextHimeUtils:
    def __init__(self, bot: commands.Bot = None, ctx=None, config=None):
        ...
        self.ctx = ctx
        self.mentions: str = ""
        self.temp_path = ("/tmp" if platform.system() == "Darwin" else
                          tempfile.gettempdir())
        self.config = config

    def check_msg_mentions(self,
                           ctx: discord.message.Message) -> "NextHimeUtils":
        self.ctx = ctx
        self.mentions = re.findall("<@!(.*?)>",
                                   f"{ctx.content}") or re.findall(
                                       "<@(.*?)>", f"{ctx.content}")
        return self
    ...
    def replace_msg_mention(self) -> str:
        for mention in self.mentions:
            ...
            self.ctx.content = self.ctx.content.replace(
                f"<@!{mention}>", f"@{user}").replace(f"<@{mention}>",
                                                      f"@{user}")
