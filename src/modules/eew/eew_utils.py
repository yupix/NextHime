...
class EewInfo(object):
    def __init__(self, result, intensity_pref_list, locale: str = "ja"):
        ...
        self.depth: str = (
            i18n.t("message.eew.word.run_depth", locale=locale) %
            result["Body"]["Earthquake"]["Hypocenter"]["Depth"], )
        self.intensity = result["Body"]["Intensity"]["Observation"]["MaxInt"]
        self.max_intensity: str = (
            f"{result['Body']['Intensity']['Observation']['MaxInt']}".replace(
                "+", i18n.t("message.eew.intensity_strong",
                            locale=locale)).replace(
                                "-",
                                i18n.t("message.eew.intensity_weak",
                                       locale=locale)))
...
class EewMessage(object):
    ...
    def get_color(self, intensity: str):
        color = int(
            self.color_list[f"{intensity}".replace("-", "").replace("+", "")],
            base=16,
        )
        return color
    ...
    def create_main_embed(self, image_url: str):
        self.embed = EmbedManager(ctx=self.ctx).generate(
            ...
            embed_description=i18n.t("message.eew.word.headline",
                                     locale=self.locale) % self.eew.headline,
            embed_content=[
                {
                    "title":
                    i18n.t("message.eew.word.epicenter", locale=self.locale),
                    "value":
                    f"{self.eew.epicenter}",
                    "option": {
                        "inline": "True"
                    },
                },
                {
                    "title":
                    i18n.t("message.eew.word.magnitude", locale=self.locale),
                    "value":
                    f"{self.eew.magnitude}",
                    "option": {
                        "inline": "True"
                    },
                },
                {
                    "title": i18n.t("message.eew.word.depth",
                                    locale=self.locale),
                    "value": f"{self.eew.depth}",
                    "option": {
                        "inline": "True"
                    },
                },
                {
                    "title":
                    i18n.t("message.eew.word.max_depth", locale=self.locale),
                    "value":
                    f"{self.eew.max_intensity}",
                    "option": {
                        "inline": "True"
                    },
                },
        ...
        for intensity_pref in self.eew.intensity_pref_list:
            ...
            for intensity_pref_area in intensity_pref["Area"]:
                for intensity_pref_area_city in intensity_pref_area["City"]:
                    intensity_pref_area_city_list += (
                        f"{intensity_pref_area_city['Name']}, ")
            await EmbedManager(self.ctx).generate(
                embed_title=i18n.t("message.eew.word.info",
                                   locale=self.locale),
                embed_content=[
                    {
                        "title":
                        i18n.t("message.eew.word.epicenter",
                               locale=self.locale),
                        "value":
                        f'{intensity_pref["Name"]}',
                        "option": {
                            "inline": "True"
                        },
                    },
                    {
                        "title":
                        i18n.t("message.eew.word.max_depth",
                               locale=self.locale),
                        "value":
                        f"{self.eew.magnitude}",
                        "option": {
                    ...
                    {
                        "title":
                        i18n.t("message.eew.word.surrounding_area",
                               locale=self.locale),
                        "value":
                        f"{intensity_pref_area_city_list[:-1]}",
                        "option": {
                            "inline": "True"
                        },
                    },
