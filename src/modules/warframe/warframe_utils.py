...
class WarframeFissure:
    def __init__(self, fissure_dict):
        ...
        self.tier: int = int([
            key for key, value in tier_int_list.items()
            if value == fissure_dict["tier"]
        ][0])
...
class WarframeTranslator(object):
    ...
    async def translate(self, fissure):
        ...
        """
        mission_type = LanguageManager(
            base_path="./src/language/",
            lang=f"{self.lang}",
            module_name="/warframe/mission_type.yml",
        ).get()
        try:
            fissure.missionType = fissure.missionType.replace(
                fissure.missionType,
...
class WarframeChannels(object):
    ...
    async def send_message(self, channels):
        ...
        for channel in channels:
            try:
                temp_fissure = await WarframeTranslator(
                    channel.region).translate(self.fissure)
                channel = self.bot.get_channel(channel.channel_id)
                await EmbedManager(channel).generate(
                    self.fissure.node,
                    f"ミッション内容: {temp_fissure.missionType}",
                    embed_content=[
                        {
                            "title": "出現エネミー",
                            "value": f"{self.fissure.enemy}"
                        },
                        {
                            "title": "レリック",
                ...
                await EmbedManager(channel).generate(
                    self.fissure.node,
                    i18n.t("message.warframe.NotSupportedType",
                           locale="ja").format(type=self.fissure.missionType),
                    mode="failed",
                ).send()
